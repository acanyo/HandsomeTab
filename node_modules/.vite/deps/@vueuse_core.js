...
function reactiveOmit(obj, ...keys2) {
  ...
  return reactiveComputed(() => typeof predicate === "function" ? Object.fromEntries(Object.entries(toRefs(obj)).filter(([k, v]) => !predicate(toValue2(v), k))) : Object.fromEntries(Object.entries(toRefs(obj)).filter((e) => !flatKeys.includes(e[0]))));
}
...
function refAutoReset(defaultValue, afterMs = 1e4) {
  return customRef((track, trigger) => {
    let value = toValue2(defaultValue);
    let timer;
    const resetAfter = () => setTimeout(() => {
      value = toValue2(defaultValue);
      trigger();
    }, toValue2(afterMs));
    tryOnScopeDispose(() => {
      clearTimeout(timer);
...
function createUntil(r, isNot = false) {
  function toMatch(condition, { flush = "sync", deep = false, timeout, throwOnTimeout } = {}) {
    ...
    if (timeout != null) {
      promises.push(
        promiseTimeout(timeout, throwOnTimeout).then(() => toValue2(r)).finally(() => stop == null ? void 0 : stop())
      );
    }
...
function useArrayDifference(...args) {
  ...
  const diff1 = computed(() => toValue2(list).filter((x) => toValue2(values).findIndex((y) => compareFn(x, y)) === -1));
  if (symmetric) {
    const diff2 = computed(() => toValue2(values).filter((x) => toValue2(list).findIndex((y) => compareFn(x, y)) === -1));
    return computed(() => symmetric ? [...toValue2(diff1), ...toValue2(diff2)] : toValue2(diff1));
  } else {
    return diff1;
  }
}
function useArrayEvery(list, fn) {
  return computed(() => toValue2(list).every((element, index, array) => fn(toValue2(element), index, array)));
}
function useArrayFilter(list, fn) {
  return computed(() => toValue2(list).map((i) => toValue2(i)).filter(fn));
}
function useArrayFind(list, fn) {
  return computed(() => toValue2(
    toValue2(list).find((element, index, array) => fn(toValue2(element), index, array))
  ));
}
function useArrayFindIndex(list, fn) {
  return computed(() => toValue2(list).findIndex((element, index, array) => fn(toValue2(element), index, array)));
}
...
function useArrayIncludes(...args) {
  ...
  if (typeof comparator === "string") {
    ...
    comparator = (element, value2) => element[key] === toValue2(value2);
  }
  comparator = comparator != null ? comparator : (element, value2) => element === toValue2(value2);
  return computed(() => toValue2(list).slice(formIndex).some((element, index, array) => comparator(
    toValue2(element),
    toValue2(value),
    index,
    toValue2(array)
...
function useArrayJoin(list, separator) {
  return computed(() => toValue2(list).map((i) => toValue2(i)).join(toValue2(separator)));
}
function useArrayMap(list, fn) {
  return computed(() => toValue2(list).map((i) => toValue2(i)).map(fn));
}
function useArrayReduce(list, reducer, ...args) {
  const reduceCallback = (sum, value, index) => reducer(toValue2(sum), toValue2(value), index);
  return computed(() => {
    const resolved = toValue2(list);
    return args.length ? resolved.reduce(reduceCallback, typeof args[0] === "function" ? toValue2(args[0]()) : toValue2(args[0])) : resolved.reduce(reduceCallback);
  });
}
function useArraySome(list, fn) {
  return computed(() => toValue2(list).some((element, index, array) => fn(toValue2(element), index, array)));
}
function uniq(array) {
...
function formatDate(date, formatStr, options = {}) {
  ...
  const matches = {
    ...
    MMM: () => date.toLocaleDateString(toValue2(options.locales), { month: "short" }),
    MMMM: () => date.toLocaleDateString(toValue2(options.locales), { month: "long" }),
    D: () => String(days),
    ...
    d: () => day,
    dd: () => date.toLocaleDateString(toValue2(options.locales), { weekday: "narrow" }),
    ddd: () => date.toLocaleDateString(toValue2(options.locales), { weekday: "short" }),
    dddd: () => date.toLocaleDateString(toValue2(options.locales), { weekday: "long" }),
...
function useDateFormat(date, formatStr = "HH:mm:ss", options = {}) {
  return computed(() => formatDate(normalizeDate(toValue2(date)), toValue2(formatStr), options));
}
...
function useToString(value) {
  return computed(() => `${toValue2(value)}`);
}
...
function watchArray(source, cb, options) {
  let oldList = (options == null ? void 0 : options.immediate) ? [] : [...source instanceof Function ? source() : Array.isArray(source) ? source : toValue2(source)];
      ...
      get(target2, prop, rec) {
        ...
        if (!(prop in refs)) {
          if (/[+_-]/.test(prop)) {
            ...
            refs[prop] = computed(() => keys2.every((key) => toValue2(proxy[key])));
          } else {
            refs[prop] = ref(false);
          }
        }
...
function useClamp(value, min, max) {
  if (typeof value === "function" || isReadonly(value))
    return computed(() => clamp(toValue2(value), toValue2(min), toValue2(max)));
...
function useSorted(...args) {
  ...
  if (!dirty)
    return computed(() => sortFn([...toValue2(source)], compareFn));
  watchEffect(() => {
...
function useTimeAgo(time, options = {}) {
  ...
  const timeAgo = computed(() => formatTimeAgo(new Date(toValue2(time)), options, toValue2(now2)));
  if (exposeControls) {
    return {
      timeAgo,
      ...controls
